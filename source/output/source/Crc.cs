...
namespace GehtSoft.DocCreator.Output
{
    public class Crc
    {
        public static string GetCrc(string source)
        {
            ...
            while(crc != 0)
            {
                ...
                if (digit < 10)
                    ...
                    output.Append((char)('A' + digit - 10));
                else
                    output.Append((char)('a' + digit - 36));
            }
        ...
        static uint crc32(byte[] buff)
        {
            uint crc = 0;
            for (int i = 0; i < buff.Length; i++)
                crc = (crc << 8) ^ crc32_table[((crc >> 24) ^ buff[i]) & 0xff];
            return crc;
        }
    }
}
