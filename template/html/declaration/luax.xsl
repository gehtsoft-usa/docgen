...
<xsl:choose>
    ...
    <xsl:when test="../@type='method'">
        <xsl:value-of select="ext:let('decl-body', concat(ext:get('prefix'), 'function ', '&lt;b&gt;', ext:get('name'), ext:get('name-suffix'), '&lt;/b&gt; (', ext:get('params'), ')', ' : ', ext:get('return'), ext:get('suffix'))) " />
    </xsl:when>
    <xsl:when test="../@type='property'">
        <xsl:value-of select="ext:let('decl-body', concat(ext:get('prefix'), 'var ', '&lt;b&gt;', ext:get('name'), ext:get('name-suffix'), '&lt;/b&gt;', ' : ', ext:get('return'), ext:get('suffix'))) " />
    </xsl:when>
</xsl:choose>
<!-- write declaration -->
<table class="decl">
